...
class UringStoreTest : public ::testing::Test {
protected:
    void SetUp() override {
        path_ = std::filesystem::temp_directory_path() /
                "udepot_uring_store_test";
...
TEST_F(UringStoreTest, GetNonexistentKeyReturnsNotFound) {
    uint8_t val[64];
    size_t val_size = 0;
...
TEST_F(UringStoreTest, PutMultipleKeysGetEach) {
...
TEST_F(UringStoreTest, LargeValueRoundTrips) {
...
TEST_F(UringStoreTest, DeleteRemovesKey) {
...
TEST_F(UringStoreTest, DeleteNonexistentKeyReturnsNotFound) {
    int rc = store_.del("nosuchkey").run_sync();
    EXPECT_NE(rc, 0);
}

TEST_F(UringStoreTest, ExistsReturnsSizeForPresentKey) {
...
TEST_F(UringStoreTest, ExistsReturnsNotFoundForMissingKey) {
    size_t val_size = 0;
    int rc = store_.exists("missing", &val_size).run_sync();
    EXPECT_NE(rc, 0);
}

TEST_F(UringStoreTest, ManyKeysRoundTrip) {
