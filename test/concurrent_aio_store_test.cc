...
class ConcurrentAioStoreTest : public ::testing::Test {
protected:
    void SetUp() override {
        path_ = std::filesystem::temp_directory_path() /
                "udepot_concurrent_aio_store_test";
...
TEST_F(ConcurrentAioStoreTest, ConcurrentPutsThenVerify) {
...
TEST_F(ConcurrentAioStoreTest, ParallelReadsReturnCorrectData) {
    ...
    EXPECT_EQ(mismatches.load(), 0)
        << "parallel AIO reads returned wrong data";
    EXPECT_EQ(total_reads.load(), static_cast<uint64_t>(kKeys));
}

TEST_F(ConcurrentAioStoreTest, ConcurrentReadersAndWriters) {
...
TEST_F(ConcurrentAioStoreTest, ConcurrentPutDeleteGet) {
    constexpr int kThreads = 4;
    ...
    // Phase 2: even threads delete, odd threads read.
    for (int t = 0; t < kThreads; ++t) {
        threads.emplace_back([&, t] {
            if (t % 2 == 0) {
                for (int i = 0; i < kOpsPerThread; ++i) {
    ...
    // Phase 3: verify deleted keys gone, surviving keys intact.
    for (int t = 0; t < kThreads; ++t) {
        for (int i = 0; i < kOpsPerThread; ++i) {
            std::string key = make_key(t, i);
            uint8_t val[128];
...
TEST_F(ConcurrentAioStoreTest, ConcurrentExistsWhileWriting) {
    constexpr int kWriters = 2;
